/* Project Name: App Trader Analysis
   Project Team: Rachel Kocheran, Morgan Hines, Adam Crank */




/* Analysis for the most popular apps regarding content_rating in the Android Play Store.
...
/* Analysis for the most popular apps regarding content_rating in the Apple App Store.
...
/* Establish a temporary table which joins both the app and play store databases, removes unnecessary duplicates
   and adds calucated columns for each distinct app's average price, rating, review_count, and purchase price with a final
   calculated column for each app's estimated lifespan. */
...
WITH distinct_appstores AS (    
	SELECT DISTINCT name,
	 	 ...
	 	 CASE WHEN ((app_store_apps.price::MONEY + play_store_apps.price::MONEY)/2)::NUMERIC >=2.5
...
/* Determine the best recommendations for App Trader using the most popular content ratings, genres and price brackets,
   and filter the results to return the Top-10 most profitable apps based on the calculated fields for net profit over each 
...
WHERE avg_price::NUMERIC < 2 
      AND app_genre ILIKE('%game%') 
      AND play_genre ILIKE('%game%')
	AND app_content_rating = '4+' 
      AND play_content_rating = 'Everyone'
...
/* CTEs for determining a list of top-10 app recommendations for Halloween 
   based on profitability and theme. */
...
/* Filter for the Top-5 Halloween app recommendations using the above two CTEs. */
